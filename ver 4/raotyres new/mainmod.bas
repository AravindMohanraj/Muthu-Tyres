...
Public rsTxM As ADODB.Recordset, rsBC As ADODB.Recordset, rsTax As ADODB.Recordset
...
Public cCash As String, cBank As String, cSales As String, cPurchase As String, cSales2 As String, cOtherIncome As String
...
Public cServiceTax As String, cSalestax1 As String, cSalestax2 As String, cSalestax3 As String, cSalestax4 As String
Public cSalestax5 As String, cASC As String, cAT As String, cCST As String, cExcise As String
Public Sub FillTax(oGrid As MSFlexGrid)
Dim nR As Integer
Set rsTax = New ADODB.Recordset
oGrid.Clear
oGrid.Rows = 2
oGrid.FormatString = "Tax                                 | Code   |  Tax P "

rsTax.Open "select * from tax where taxp>0", Con, adOpenStatic
If Not rsTax.EOF Then
nR = 1
If Not rsTax.BOF Then rsTax.MoveFirst
    Do While Not rsTax.EOF
       
           oGrid.TextMatrix(nR, 0) = rsTax!display
               oGrid.TextMatrix(nR, 1) = rsTax!faccode
                   oGrid.TextMatrix(nR, 2) = rsTax!taxp
    oGrid.AddItem ""
    nR = nR + 1
    rsTax.MoveNext
    Loop
...
End Function
Public Function LPadS(cNum As String, nLen As Integer, cPadString As String) As String
Dim CPad As String, nlStr As Integer
CPad = Left(cNum, nLen)
nlStr = nLen - Len(CPad)
LPadS = String(nlStr, " ") + Left(cNum, nLen)
End Function
       ...
       If Not rsTemp.BOF Then rsTemp.MoveFirst
        Do While Not rsTemp.EOF
           Select Case rsTemp!ftemp
             ...
             Case Is = "VAT"  ' TODO use it for single tax
                  cVat = rsTemp!fcode
             Case Is = "TRANS"
                  cTrans = rsTemp!fcode
             Case Is = "CARD"
             ...
             Case Is = "SALES2"
                   cSales2 = rsTemp!fcode
             Case Is = "OTHER INCOME"
                   cOtherIncome = rsTemp!fcode
           End Select
        rsTemp.MoveNext
        Loop
    End If
...
rsTax.Open "select * from tax", Con, adOpenStatic
If Not rsTax.EOF Then
    Do While Not rsTax.EOF
    If Not IsNull(rsTax!faccode) Then
        If rsTax!TAXCODE = "SERVICE" Then
         cServiceTax = rsTax!faccode
        ElseIf rsTax!TAXCODE = "ST1" Then
         cSalestax1 = rsTax!faccode
        ElseIf rsTax!TAXCODE = "ST2" Then
         cSalestax1 = rsTax!faccode
        ElseIf rsTax!TAXCODE = "ST4" Then
         cSalestax1 = rsTax!faccode
        ElseIf rsTax!TAXCODE = "ST5" Then
         cSalestax1 = rsTax!faccode
        ElseIf rsTax!TAXCODE = "ASC" Then
         cASC = rsTax!faccode
        ElseIf rsTax!TAXCODE = "AT" Then
         cAT = rsTax!faccode
        ElseIf rsTax!TAXCODE = "CST" Then
        cCST = rsTax!faccode
        ElseIf rsTax!TAXCODE = "EXCISE DUTY" Then
        cExcise = rsTax!faccode
        End If
     End If
    rsTax.MoveNext
    Loop
