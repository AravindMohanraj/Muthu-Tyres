...
Begin VB.Form frmBill 
   ...
   Begin VB.Frame Frame3 
      ...
      Begin VB.ComboBox cboBranch 
         Height          =   315
         ItemData        =   "frmBill.frx":0064
         Left            =   90
         List            =   "frmBill.frx":0066
         Style           =   2  'Dropdown List
         TabIndex        =   0
         Top             =   435
         Width           =   1440
...
Dim cTaxDisp As String, cVatL As String
...
Dim nTlen, nLoop, nInc As Integer, cString As String, nDis_p As Integer, nVal_p As Integer
Dim nCnt As Integer
nLM = 10
cVl = " "
cHl = "-"
...
'############### Head Printing #################
  '  Print #1,
    ...
    Print #1, Space(0) + Space(24) + RPad(Left(Trim(txtBno.Text), 6), 6) + "       " + Space(4) + CPad(Mid(cboPaytype.Text, 4, Len(cboPaytype.Text)), 11) + Space(14) + dBilldt.FormattedText
'    Print #1,
...
'         If Len(flxgrd.TextMatrix(i, 1)) < 25 Then
           Print #1, Space(nLM) + Space(6) + RPad(Left(FlxGrd.TextMatrix(I, 1), 42), 42) + Space(1) + LPad(FlxGrd.TextMatrix(I, 2), 3) + Space(1) + LPad(Format(FlxGrd.TextMatrix(I, 3), "####0"), 5) + Space(0) + LPad(Format(FlxGrd.TextMatrix(I, 4), "#####0.00"), 9)
 '        Else
  '         Print #1, Space(nLM) + Space(3) + RPad(flxgrd.TextMatrix(i, 0), 2) + RPad(flxgrd.TextMatrix(i, 1), 6) + RPad(flxgrd.TextMatrix(i, 2), 5) + RPad(flxgrd.TextMatrix(i, 3), 6) + RPad(flxgrd.TextMatrix(i, 4), 6)
   '        Print #1, Space(nLM) + Space(3) + RPad(Mid(flxgrd.TextMatrix(i, 1), 15, Len(flxgrd.TextMatrix(i, 1))), 5)
    ...
    Next
        ...
        nCnt = nCnt + 1
        Print #1, Tab(nDis_p); "Product Value"; Tab(nVal_p); LPad(Format(txtSTot.Text, "#####0.00"), 9)
    
    If Val(txtDedR) <> 0 Then
        nCnt = nCnt + 1
        
        Print #1, Tab(nDis_p); "Discount"; Tab(nVal_p); LPad(Format(txtDedR, "#####0.00"), 9)
        nCnt = nCnt + 1
        Print #1, Tab(nDis_p); "Sub Total"; Tab(nVal_p); LPad(Format(Val(txtSTot.Text) - Val(txtDedR), "#####0.00"), 9)
    Else
       nCnt = nCnt + 2
       Print #1,
       Print #1,
    ...
    End If
        ...
        'Print #1, Space(nLM) + Space(1) + "VAT @ " + LPad(txtTaxP, 4) + " %" + Space(33) + LPad(Format(txtTaxR, "#####0.00"), 9)
                Print #1, Tab(nDis_p); RPad(cTaxDisp, 20); Tab(nVal_p); LPad(Format(txtTaxR, "#####0.00"), 9)

    If Val(txtBcP) <> 0 Then
        nCnt = nCnt + 1
        Print #1, Tab(nDis_p); "Bank Chg"; Tab(nVal_p); LPad(Format(txtBcR, "#####0.00"), 9)
    Else
        nCnt = nCnt + 1
       Print #1,
    End If
...
'Print #1, Tab(nVal_p); LPad(Format(txtTotal, "#####0.00"), 9)
...
Print #1, Tab(nDis_p); cWord1; Tab(nVal_p); LPad(Format(txtTotal, "#####0.00"), 9)
Print #1, Tab(nDis_p); cWord2
Else
Print #1, Tab(nDis_p); cWord; Tab(nVal_p); LPad(Format(txtTotal, "#####0.00"), 9)
End If
...
Private Sub FarTax(nTaxP, nTax)
'Find if exist
For I = 1 To tAxGrid.Rows - 1
If tAxGrid.TextMatrix(I, 0) = nTaxP Then
tAxGrid.TextMatrix(I, 1) = Val(tAxGrid.TextMatrix(I, 1)) + nTax
Exit Sub
End If
Next
tAxGrid.AddItem ""

tAxGrid.TextMatrix(tAxGrid.Rows - 1, 0) = Str(nTaxP)
tAxGrid.TextMatrix(tAxGrid.Rows - 1, 1) = Str(nTax)
End Sub
    ...
    If lSuTax Then
        ...
        FlxGrd.TextMatrix(FlxGrd.Row, 10) = IIf(IsNull(rsItem!ftax), "", rsItem!ftax)
            ...
            '====================================================================
                    ...
                    txtTaxP.Text = IIf(IsNull(rsItem!ftax), "", rsItem!ftax)
                    cVatL = IIf(IsNull(rsItem!ftaxacccode), "", rsItem!ftaxacccode)
                    FarTax IIf(IsNull(rsItem!ftax), "", rsItem!ftax), 0
                Set rsTxM = New ADODB.Recordset
                rsTxM.Open "select * from tax where val(taxp) = '" & Val(txtTaxP) & "'", Con, adOpenStatic
                      If Not rsTxM.EOF Then
                      If Not rsTxM.BOF Then rsTxM.MoveFirst
                         cTaxDisp = IIf(IsNull(rsTxM!display), "", rsTxM!display)
                    End If
                rsTxM.Close
                Set rsTxM = Nothing
...
'                    rsBLst!fddate = ""
                    ...
                    rsBLst!FtaxTypeBill = cboTaxType.ListIndex
    ...
    For I = 1 To FlxGrd.Rows - 1
        If FlxGrd.TextMatrix(I, 5) <> "" Then
            ...
            rsBill.AddNew
                ...
                rsBill!FtaxTypeBill = cboTaxType.ListIndex
            ...
            rsIled.AddNew
                ...
                rsIled!FtaxTypeBill = cboTaxType.ListIndex
                rsIled!ftag = 10
            rsIled.Update
    ...
    rsLed.Open "select * from led0203d", Con, adOpenDynamic, adLockPessimistic
                ...
                If lSuTax And cboTaxType.ListIndex = 0 Then
                    rsLed.AddNew
                    rsLed!fvrtype = nVrtype
                    rsLed!fvrno = Val(txtBno)
                    rsLed!fvrdate = dBilldt.FormattedText
                    rsLed!faccode = cVatL
                    rsLed!fcrdb = "CR"
                    rsLed!famount = Val(txtTaxR.Text)
                    If cboPaytype.ListIndex = 0 Or cboPaytype.ListIndex = 2 Then
                ...
                If lSuDed And Val(txtDedR.Text) > 0 Then
                    rsLed.AddNew
                    rsLed!fvrtype = nVrtype
                    rsLed!fvrno = Val(txtBno)
                    rsLed!fvrdate = dBilldt.FormattedText
               ...
               If lSuPc And Val(txtTrnsChg) > 0 Then
                    rsLed.AddNew
                    rsLed!fvrtype = nVrtype
                    rsLed!fvrno = Val(txtBno)
                    rsLed!fvrdate = dBilldt.FormattedText
...
Dim cBn As String
If KeyAscii = 13 Then
cboTaxType.ListIndex = CboBranch.ListIndex
   Set rsBBnum = New ADODB.Recordset
...
Private Sub cboTaxType_Change()
CboBranch.ListIndex = cboTaxType.ListIndex
...
CboBranch.ListIndex = cboTaxType.ListIndex
If cboTaxType.ListIndex = 0 Then
...
Dim nRate_o As Double, nTax_H As Double
If KeyAscii <> 13 Then
...
ElseIf KeyAscii = 13 Then
     ...
     ElseIf FlxGrd.Col = 3 Then
            ...
            nRate_o = FlxGrd.TextMatrix(FlxGrd.Row, 3)
            nTax_H = 100 + Val(txtTaxP)
            FlxGrd.TextMatrix(FlxGrd.Row, 3) = Round(((nRate_o / nTax_H) * 100), 2)
...
If KeyAscii = 13 And Val(txtAmtR) <> 0 Then
   ...
   If nOpt = 1 Then
      ...
      Else
             If MsgBox("Print the bill", vbYesNo) = vbYes Then WinPrint
...
p.FontName = "courier"
p.FontSize = 10
p.Print Space(15) + "SENTHIL TYRES"
p.FontName = "courier"
p.FontSize = 9
p.Print Space(10) + "No.225/2,Vandalur-Walajabad Road,"
p.Print Space(10) + "Salamangalam, Padappai -601 301"
p.FontName = "courier"
p.FontSize = 8

p.Print Space(5) + "Cell: 9841914046 E-mail: senthiltyres_carcare@yahoo.com"
p.FontBold = True
p.Print Space(25) + "Service Bill"
p.FontBold = False
p.FontName = "courier"
p.FontSize = 8
p.FontBold = False
 If txtNote <> "" Then p.Print Tab(0); "Vehical No :" + txtNote.Text
 If txtMdt <> "" Then p.Print Tab(0); "Note :" + txtMdt.Text

 
If txtName <> "" Then
p.Print Tab(0); RPad(Left(Trim("M/s" + txtName.Text), 22), 22); Tab(40); " Bill :" + RPad(Left(Trim(txtBno.Text), 9), 9)
ElseIf txtPartyName <> "" Then
p.Print Tab(0); RPad(Left(Trim("M/s" + txtPartyName.Text), 22), 22); Tab(40); " Bill " + RPad(Left(Trim(txtBno.Text), 9), 9)
Else
p.Print Tab(40); " Bill No :" + RPad(Left(Trim(txtBno.Text), 12), 12)
End If
If txtAdd1.Text <> "" Then
    p.Print Tab(0); RPad(Left(txtAdd1.Text, 22), 22); Tab(40); " Date :" + dBilldt.Text
Else
    p.Print Tab(40); " Date :" + dBilldt.Text
End If
If txtAdd2.Text <> "" Then
    p.Print Tab(0); RPad(Left(txtAdd2.Text, 22), 22)
    '; Tab(40); " Time :" + txtbtime.Text
Else
'    p.Print Space(nLM); Tab(40); " Time :" & txtbtime.Text
End If
If txtAdd3.Text <> "" Then p.Print Tab(0); RPad(Left(txtAdd3.Text, 37), 37)
...
p.Print Space(nLM) + String(65, "-")
p.Print Tab(0); "SNo"; Tab(6); RPad("Description", 16); Tab(29); RPad("Qty", 3); Tab(34); RPad("Rate", 7); Tab(42); RPad("Amount", 7)
p.Print Space(nLM) + String(65, "-")

For I = 1 To FlxGrd.Rows - 1
If FlxGrd.TextMatrix(I, 3) <> "" Then

'p.Print RPad(flxgrd.TextMatrix(i, 0), 3); Spc(1); RPad(Left(Trim(flxgrd.TextMatrix(i, 2)), 14), 14); Tab(26); Format(LPad(flxgrd.TextMatrix(i, 4), 7), "###0.00"); Spc(1); Format(LPad(flxgrd.TextMatrix(i, 3), 3), "##0"); Spc(1); Format(LPad(flxgrd.TextMatrix(i, 5), 9), "#####0.00")
p.Print Tab(0); RPad(FlxGrd.TextMatrix(I, 0), 3); Tab(6); RPad(Left(Trim(FlxGrd.TextMatrix(I, 1)), 16), 16); Tab(29); Format(LPad(FlxGrd.TextMatrix(I, 2), 3), "##0"); Tab(34); Format(LPad(FlxGrd.TextMatrix(I, 3), 7), "###0.00"); Tab(42); Format(LPad(FlxGrd.TextMatrix(I, 4), 7), "###0.00")

'p.Print Tab(0); RPad(flxgrd.TextMatrix(i, 0), 3); Tab(4); RPad(Left(Trim(flxgrd.TextMatrix(i, 2)), 14), 14); Tab(23); LPad(Format(flxgrd.TextMatrix(i, 4), "###0.00"), 7); Tab(32); LPad(Format(flxgrd.TextMatrix(i, 3), "##0"), 3); Tab(37); LPad(Format(flxgrd.TextMatrix(i, 5), "#####0.00"), 9)
...
If Val(txtTrnsChg) > 0 Then p.Print Tab(30); "Dely. Chgs RS."; Tab(40); LPadS(Format(txtTrnsChg, "###0.00"), 7, "#")
If Val(txtd) > 0 Then p.Print Tab(30); "Deduction RS. "; Tab(40); LPadS(Format(txtDedR, "###0.00"), 7, "#")
If Val(txtTaxP) >= 0 Then p.Print Tab(10); cTaxDisp; Tab(40); LPadS(Format(txtTaxR, "###0.00"), 7, "#")

p.FontName = "courier"
p.FontSize = 12
p.FontBold = True
p.Print Tab(0); "Total Amount " + LPadS(Format(txtTotal, "###0.00"), 7, "#")
p.Print
'p.Print cUser
p.EndDoc
